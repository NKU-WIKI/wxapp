...
export const fetchUnreadCounts = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      const promises = notificationTypes.map(async (type) => {
        try {
          ...
          if (res.code === 0 && res.data?.pagination) {
            // 根据business_type重新过滤通知，确保统计正�?            let items = (res.data.items || []);
            ...
            const unreadItems = items.filter((item) => {
              // 支持多种已读状态表示方�?              const readStatuses = ['read', 'Read', 'READ'];
            ...
            //   business_type过滤�? items.length,
            //   最终未读数�? count,
            //   未读通知状�? unreadItems.map(item => ({ id: item.id, status: item.status, business_type: item.business_type })),
            //   type
            // });
      ...
      // // console.log('🔧 [Redux调试] 最终未读数量统�?, finalUnreadCounts);
...
const notificationSlice = createSlice({
  ...
  reducers: {
    ...
    updateUnreadCounts: (
      state,
      action: PayloadAction<NotificationState["unreadCounts"]>,
    ) => {
    ...
    markTypeAsRead: (state, action: PayloadAction<NotificationType>) => {
      ...
      if (state.unreadCounts[type]) {
        ...
        state.unreadCounts.total = Math.max(
          0,
          (state.unreadCounts.total || 0) - typeCount,
        );
    ...
    incrementUnreadCount: (
      state,
      action: PayloadAction<{ type: NotificationType; count?: number }>,
    ) => {
