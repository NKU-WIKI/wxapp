...
export interface SettingsState {
  ...
  allowImageSaving: boolean; // 新增：是否允许他人保存图片
...
const defaultSettings: SettingsState = {
  ...
  allowImageSaving: true, // 新增：默认允许保存图片
  isInitialized: false,
};
...
export const SETTINGS_STORAGE_KEYS = {
  ...
  ALLOW_IMAGE_SAVING: "settings_allow_image_saving", // 新增
};
...
const loadSettingsFromStorage = (): Partial<SettingsState> => {
  try {
    ...
    // 加载布尔值设置
    const messageNotification = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.MESSAGE_NOTIFICATION,
    );
    const pushNotification = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.PUSH_NOTIFICATION,
    );
    const privateMessage = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.PRIVATE_MESSAGE,
    );
    const personalizedRecommendation = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.PERSONALIZED_RECOMMENDATION,
    );
    const allowFileUpload = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.ALLOW_FILE_UPLOAD,
    );
    const allowClipboardAccess = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.ALLOW_CLIPBOARD_ACCESS,
    );
    const allowImageSaving = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.ALLOW_IMAGE_SAVING,
    ); // 新增

    if (messageNotification !== "")
      settings.messageNotification = messageNotification;
    ...
    if (allowClipboardAccess !== "")
      settings.allowClipboardAccess = allowClipboardAccess;
    if (allowImageSaving !== "") settings.allowImageSaving = allowImageSaving; // 新增
    ...
    const whoCanMessage = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.WHO_CAN_MESSAGE,
    );
    const whoCanComment = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.WHO_CAN_COMMENT,
    );
    const whoCanViewPosts = Taro.getStorageSync(
      SETTINGS_STORAGE_KEYS.WHO_CAN_VIEW_POSTS,
    );
...
// 保存设置到本地存储
const saveSettingToStorage = (key: string, value: unknown) => {
  try {
...
const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    setWhoCanMessage: (
      state,
      action: PayloadAction<"everyone" | "followers" | "none">,
    ) => {
    ...
    setWhoCanViewPosts: (
      state,
      action: PayloadAction<"everyone" | "followers" | "self">,
    ) => {
