...
const CustomTabBar: FC = () => {
  ...
  const list = useMemo(
    () => [
      ...
      {
        pagePath: '/pages/subpackage-interactive/publish/index',
        text: '发布',
        iconPath: plusIcon,
  ...
  return (
    <>
      {/* 灰色滤镜遮罩 */}
      {showSubMenu && <View className={styles.overlay} onClick={handleOverlayClick} />}

      {/* 子菜单 */}
      {showSubMenu && (
        <View className={styles.subMenuContainer}>
          <View
            className={`${styles.subMenuItem} ${styles.postItem}`}
            onClick={() => handleSubMenuClick('post')}
          >
          ...
          <View
            className={`${styles.subMenuItem} ${styles.noteItem}`}
            onClick={() => handleSubMenuClick('note')}
          >
            <View className={styles.textContent}>
          ...
          >
            {item.isPublish ? (
              ...
              <>
                ...
                />
                <View className={`${styles.text} ${selected === index ? styles.selected : ''}`}>
                  {item.text}
                </View>
              </>
            )}
