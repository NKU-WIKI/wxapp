...
const parseMarkdown = (markdown: string): string => {
  ...
  let html = processedMarkdown
    ...
    .replace(
      /- (.*$)/gim,
      '<span class="markdown-list-item">• <strong>$1</strong></span>',
    )
    // 有序列表 - 改为带点的加粗格式
    .replace(
      /(\d+)\. (.*$)/gim,
      '<span class="markdown-list-item">• <strong>$2</strong></span>',
    )
    // 段落分隔（双换行转为段落分隔）
  ...
  if (html.trim()) {
    // 如果内容以块级元素开头，不需要额外包装
    if (
      !html.startsWith('<h1 class="markdown-h1">') &&
    ...
    return (
      html
        ...
        .replace(
          ...
          '<pre class="code-block lang-$1"><code>',
        )
