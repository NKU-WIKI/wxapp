...
export const getFollowers = async (params: GetFollowersParams) => {
  ...
  try {
    ...
    // 使用用户ID构建正确的端点
    const endpoint =
      params.type === 'following' ? `/users/${userId}/following` : `/users/${userId}/followers`
 ...
 * @returns
 */
export const getUserFollowers = (userId: string, params: PaginationParams) => {
  return http.get<User[]>(`/users/${userId}/followers`, params)
}
 ...
 * @returns
 */
export const getUserFollowing = (userId: string, params: PaginationParams) => {
  return http.get<User[]>(`/users/${userId}/following`, params)
}
