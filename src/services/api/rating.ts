...
export const getResourceRatings = (
  ...
  return http
    .get<
      RatingApiResponse<ResourceRatingsResponse>
    >(`/ratings/resources/${resourceType}/${resourceId}`, filteredParams)
    .then((response) => {
      // 处理分页参数格式转换
      if (response.data?.data) {
        ...
        if (
          (data as DataWithPagination).skip !== undefined &&
          (data as DataWithPagination).limit !== undefined
        ) {
          (data as DataWithPagination).page =
            Math.floor(
              ((data as DataWithPagination).skip ?? 0) /
                ((data as DataWithPagination).limit ?? 20),
            ) + 1;
          (data as DataWithPagination).page_size = (
            data as DataWithPagination
          ).limit;
        }
      }
      return response;
...
export const getRatingItems = (params: GetRatingItemsRequest = {}) => {
  ...
  return http
    .get<RatingApiResponse<ResourceRatingsResponse>>("/ratings", queryParams)
    .then((response) => {
      // 直接使用API返回的原始数据结构，不进行转换
      const apiData = response.data?.data || response.data || {};
      const items = (apiData as { items?: unknown[] }).items || [];
      const total = (apiData as { total?: number }).total || 0;
      const currentPage = Math.floor(queryParams.skip / queryParams.limit) + 1;

      // 为了兼容前端组件，我们保留原始数据并添加必要的字段
      const processedItems = items.map((item: UserRating) => ({
        // 保留原始API数据结构
        ...item,
        // 添加一些兼容字段（如果前端其他地方需要）
        _originalData: item,
    ...
    .catch((error) => {
      if (error.statusCode) {
        console.error('API错误状态码:', error.statusCode);
      }
      if (error.data) {
        console.error('API错误数据:', error.data);
      }
      throw error;
    });
};
...
export const getRatingItemDetail = (itemId: string) => {
  return http.get<RatingApiResponse<RatingItemDetailResponse>>(
...
export const createRatingItem = (data: CreateRatingItemRequest) => {
  return http.post<RatingApiResponse<RatingItem>>("/ratings/items", data);
};
...
export const createRating = (data: CreateRatingRequest) => {
  ...
  return http.post<RatingApiResponse<UserRating>>("/ratings", requestData);
};
...
export const submitRating = (data: SubmitRatingRequest) => {
  return http.post<RatingApiResponse<UserRating>>("/ratings", data);
};
...
export const getRatingStatistics = (itemId: string) => {
  return http.get<RatingApiResponse<RatingStatistics>>(
    `/ratings/items/${itemId}/statistics`,
  );
};
 ...
 * @returns 更新结果
 */
export const updateRatingItem = (
  itemId: string,
  data: Partial<CreateRatingItemRequest>,
) => {
...
export const deleteRatingItem = (itemId: string) => {
  return http.delete<RatingApiResponse<Record<string, unknown>>>(
    `/ratings/items/${itemId}`,
  );
};
 ...
 * @returns 热门项目列表
 */
export const getPopularRatingItems = (
 ...
 * @returns 搜索结果
 */
export const searchRatingItems = (
  keyword: string,
  params: Omit<GetRatingItemsRequest, "keyword"> = {},
) => {
...
export const getResourceList = (params: GetResourceListRequest) => {
  ...
  return http
    .get<RatingApiResponse<ResourceListResponse>>(
...
export const getResourceRatingsList = (
  ...
  return http
    .get<RatingApiResponse<ResourceRatingsResponse>>(
    ...
    .then((response) => {
      ...
      if (response.data?.data) {
        ...
        if (
          paginationData.skip !== undefined &&
          paginationData.limit !== undefined
        ) {
          paginationData.page =
            Math.floor(paginationData.skip / paginationData.limit) + 1;
          paginationData.page_size = paginationData.limit;
        }
      }
      return response;
    })
...
export const getResourceStatistics = (
  ...
  return http
    .get<RatingApiResponse<RatingStatistics>>(
 ...
 * @returns 热门资源列表
 */
export const getTopRatedResources = (
  params: GetTopRatedResourcesRequest = {},
) => {
  // 构建查询参数
  const queryParams = {
    resource_type: params.resource_type,
  ...
  return http
    .get<RatingApiResponse<RatableResource[]>>(
...
export const getMyRatings = (
  params: { resource_type?: string; skip?: number; limit?: number } = {},
) => {
  const queryParams = {
    resource_type: params.resource_type,
    skip: params.skip || 0,
  ...
  return http
    .get<RatingApiResponse<ResourceRatingsResponse>>("/ratings/me", queryParams)
    .then((response) => {
      return response;
    })
    .catch((error) => {
