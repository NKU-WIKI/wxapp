...
export const quickAnalyze = (content: string) => {
  return http.get<WorkflowExecutionResponse>(`/agent/workflow/quick-analyze?content=${encodeURIComponent(content)}`);
};

// 快速文章生成
export const quickGenerate = (topic: string) => {
  return http.get<WorkflowExecutionResponse>(`/agent/workflow/quick-generate?topic=${encodeURIComponent(topic)}`);
};
