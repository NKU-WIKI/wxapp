...
const PostItem = ({ post, className = "" }: PostItemProps) => {
  ...
  const navigateToProfile = (e) => {
    ...
    if (post.user.id === userInfo?.id) {
      ...
      // 如果是其他用户，跳转到用户详情页，传递用户信息
      const userParams = {
        userId: post.user.id,
        nickname: encodeURIComponent(post.user.nickname || '未知用户'),
        avatar: post.user.avatar || '',
        bio: encodeURIComponent(post.user.bio || ''),
        level: post.user.level || 1,
        follower_count: post.user.follower_count || 0,
        following_count: post.user.following_count || 0,
        post_count: post.user.post_count || 0,
      };
