...
export default function GeminiReadingAnimation({ sources, onComplete, duration = 4000, shouldStop = false }: GeminiReadingAnimationProps) {
    ...
    <View className={styles.container}>
      ...
      <View className={styles.headerCard}>
        ...
        <View className={styles.titleSection}>
          ...
          <View className={styles.thinkingStep}>
            ...
            <Text className={styles.thinkingText}>{isGenerating ? '正在生成最终回复...' : thinkingSteps[thinkingStepIndex]}</Text>
          </View>
      ...
      <View className={`${styles.sourceCard} ${isFading ? styles.fading : ''}`}>
        ...
        <View className={styles.sourceMeta}>
          {currentSource.author && <Text className={styles.metaItem}>👤 {currentSource.author}</Text>}
          {currentSource.publish_time && <Text className={styles.metaItem}>📅 {new Date(currentSource.publish_time).toLocaleDateString('zh-CN')}</Text>}
          {currentSource.platform && <Text className={styles.metaItem}>🌐 {currentSource.platform}</Text>}
        </View>
      ...
      <View className={styles.footerStatus}>
        <Text>{isGenerating ? '所有文献研读完毕' : `正在研读第 ${currentSourceIndex + 1} 个文献，共 ${sources.length} 个`}</Text>
      </View>
    </View>
  );
}
