...
const isNotificationRead = (
  status: NotificationStatus | string | undefined,
): boolean => {
  // 支持多种已读状态表示方�?
...
const NotificationPage = () => {
  ...
  const unreadCounts = useSelector(
    (state: RootState) => state.notification.unreadCounts,
  );
  ...
  const [initialized, setInitialized] = useState(false); // 初始化标�?
  ...
  const parseNotificationDisplay = useCallback(
    async (notification: NotificationRead) => {
      ...
      if (notification.type === NotificationType._Message) {
        ...
        // 对于非关注类型，显示相关内容
        if (
          notification.business_type !== "follow" &&
  ...
  const processNotificationDisplayData = useCallback(
    async (notificationItems: NotificationRead[]) => {
      try {
        const displayDataPromises = notificationItems.map(
          async (notification) => {
    ...
    try {
      ...
      if (isUnread) {
        ...
        setDisplayNotifications((prev) =>
          prev.map((n) =>
            n.id === originalNotification.id ? { ...n, unread: false } : n,
          ),
        );
  ...
  return (
    <View style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      <View style={{ flex: 1, overflow: "hidden" }}>
                ...
                {displayNotifications.length > 0 ? (
                  ...
                  <View className={styles.emptyState}>
                    <View className={styles.emptyIcon}>
                      {NOTIFICATION_TABS.find((tab) => tab.key === currentTab)
                        ?.icon || "💬"}
                    </View>
        ...
        {/* 底部操作按钮 - 固定在底�?*/}
        {notifications.filter((n) => !isNotificationRead(n.status)).length >
          0 && (
