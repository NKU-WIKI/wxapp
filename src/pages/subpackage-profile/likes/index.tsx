...
const LikeItemComponent: React.FC<LikeItemProps> = ({ like, isFollowingAuthor = false }) => {
  ...
  if (like.content && like.target_type === 'post') {
    ...
    const authorInfo: User | undefined = like.content.author_info
      ? {
          id: like.content.author_info.id,
          tenant_id: '', // 暂时为空
          created_at: '',
          updated_at: '',
          nickname: like.content.author_info.nickname,
          avatar: like.content.author_info.avatar,
    ...
    const postData = {
      ...
      user: authorInfo
        ? {
            id: authorInfo.id,
            tenant_id: '', // 暂时为空，后续可以从API获取
            created_at: authorInfo.created_at || '',
            updated_at: authorInfo.updated_at || '',
            ...
            filteredLikes
              ...
              .map((like, index) => {
                ...
                const noteData = {
                  ...
                  author_name: like.content.author_info?.nickname || '未知用户',
                  author_avatar: like.content.author_info?.avatar || '',
                  like_count: like.content.like_count || 0,
                  comment_count: like.content.comment_count || 0,
                  view_count: like.content.view_count || 0,
                  created_at: like.content.created_at
                    ? new Date(like.content.created_at)
