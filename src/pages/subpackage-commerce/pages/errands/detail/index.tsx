...
import { ListingRead, ErrandRead, ErrandType } from '@/types/api/marketplace.d';
...
// 类型守卫：检查是否为跑腿任务数据
const isErrandData = (data: unknown): data is ErrandRead => {
  return typeof data === 'object' && data !== null && 'errand_type' in data;
};

// 安全提取跑腿任务字段的辅助函数
const extractErrandFields = (errand: ListingRead | ErrandRead) => {
  const errandData = errand as unknown as Record<string, unknown>;

  return {
    errandType: isErrandData(errandData)
      ? (errandData.errand_type as ErrandType)
      : (errandData.errand_type as string) ||
        ('listing_type' in errand ? errand.listing_type : undefined),
    reward: isErrandData(errandData)
      ? (errandData.reward as number)
      : typeof errandData.reward === 'number' || typeof errandData.reward === 'string'
        ? errandData.reward
        : 'price' in errand
          ? errand.price
          : undefined,
    locationFrom:
      (errandData.location_from as string) || ('location' in errand ? errand.location : undefined),
    locationTo: errandData.location_to as string,
    deadline: errandData.deadline as string,
    status: isErrandData(errandData)
      ? (errandData.status as string)
      : 'status' in errand
        ? errand.status
        : undefined,
  };
...
const ErrandsDetailPage = () => {
  ...
  // 任务信息组件
  const TaskInfo = ({ errand }: { errand: ListingRead | ErrandRead }) => {
    const { formattedTime } = useRelativeTime(errand.created_at, {
      autoUpdate: true,
      updateInterval: 60000, // 每分钟更新一次
    });

    const errandOwnerId = errand.user_id || errand.user?.id;
    ...
    // 使用安全的字段提取函数
    const { errandType, reward, locationFrom, locationTo, deadline } = extractErrandFields(errand);
      ...
      <View className={styles.taskInfo}>
        <View className={styles.taskHeader}>
          <View className={styles.taskHeaderMain}>
            ...
            <Text className={styles.taskReward}>¥{reward ? reward : '面议'}</Text>
          </View>
          {isOwner && (
        ...
        {(locationFrom || locationTo) && (
          <View className={styles.taskRoute}>
            ...
            <Text className={styles.routeText}>
              {String(locationFrom || '起始地点')} → {String(locationTo || '目的地点')}
            </Text>
          </View>
        )}
        ...
        <View className={styles.taskStats}>
          <Text className={styles.statsText}>
            {'view_count' in errand ? errand.view_count || 0 : 0} 次浏览 ·{' '}
            {'favorite_count' in errand ? errand.favorite_count || 0 : 0} 人收藏
          </Text>
        </View>
      </View>
    );
  ...
  // 操作按钮组件
  const ActionButtons = ({ errand }: { errand: ListingRead | ErrandRead }) => {
    const { status: taskStatus } = extractErrandFields(errand);
      ...
      <View className={styles.actionButtons}>
        <View className={styles.leftArea}>
          <ActionBar
            ...
            initialStates={{
              'favorite-0': {
                isActive: ('favorite_count' in errand ? errand.favorite_count || 0 : 0) > 0,
                count: 'favorite_count' in errand ? errand.favorite_count || 0 : 0,
              },
            }}
