...
const ErrandsHomePage = () => {
  ...
  const loadErrands = useCallback(
    async (params?: { skip?: number; refresh?: boolean }) => {
      try {
        if (params?.refresh) {
        ...
        // 只在用户主动选择类型时才添加类型参数
        if (selectedType !== 'all') {
          queryParams.errand_type =
            selectedType === 'express'
              ? ErrandType.EXPRESS_PICKUP
              : selectedType === 'food'
                ? ErrandType.FOOD_DELIVERY
    ...
    return (
      <View className={styles.taskCard} onClick={() => handleTaskClick(task)}>
        <View className={styles.cardHeader}>
          <HighlightText text={getTaskTypeDisplay(task)} keywords={searchKeywords} />
          <Text className={styles.taskReward}>¥{task.reward}</Text>
        </View>
        <View className={styles.cardBody}>
          <HighlightText text={getTaskRouteDisplay(task)} keywords={searchKeywords} />
          <Text className={styles.deadline}>
            期望送达：{task.deadline ? new Date(task.deadline).toLocaleString() : '尽快'}
          </Text>
        </View>
        <View className={styles.cardFooter}>
          <Text className={styles.status}>{getTaskStatusDisplay(task.status)}</Text>
        </View>
      </View>
    );
  };
        ...
        >
          ...
          <View className={styles.searchContainer}>
            ...
            <View
              ...
              onClick={() =>
                Taro.navigateTo({ url: '/pages/subpackage-commerce/pages/errands/publish/index' })
              }
            >
