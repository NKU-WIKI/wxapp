      ...
      if (response.code === 0 && response.data) {
        ...
        const searchResults = response.data.items.map((item) => {
          ...
          return {
            ...
            description: item.content || '',
            price: item.price || null,
            images: item.images || ['/assets/placeholder.jpg'], // 确保至少有一个图片            condition: item.condition || '',
            category: item.category || '',
            created_at: item.created_at || item.create_time || '',
            user: {
              id: item.user_id || '',
      ...
      <View style={{ flex: 1, overflow: 'hidden' }}>
        ...
        >
          ...
          {listingsLoading === 'pending' && listings.length === 0 ? (
            ...
            <EmptyState icon={emptyIcon} text={searchKeyword ? '没有找到相关商品' : '暂无商品'} />
          )}
