...
const FilterTabs = ({
  ...
  onTypeChange: (type: 'all' | 'sell' | 'buy') => void,
}) => (
...
const handleSearch = useCallback(async () => {
  ...
  if (response.code === 0 && response.data) {
    ...
    // 设置关键词用于高亮显�?        const keywords = searchParams.q.trim().split(/\s+/).filter(k => k.length > 0)
    setSearchKeywords(keywords)

    // 批量获取商品详情（异步执行，不阻塞UI�?        const productIds = searchResults.map(item => item.id)
...
const handleRefresh = useCallback(async () => {
  // 刷新时保持当前的搜索关键�?    await loadListings({ refresh: true, keyword: searchKeyword })
...
const handleLoadMore = useCallback(async () => {
  ...
  // 传递当前搜索关键词以保持搜索状�?    await loadListings({ skip: nextSkip, keyword: searchKeyword })
