...
export default function ActivitySquare() {
  ...
  const scrollViewRef = useRef<{
    scrollTo: (options: { top: number; animated: boolean }) => void;
  } | null>(null);
  ...
  // 活动筛选选项
  const filterOptions = ['全部', '我报名的', '我发布的'];

  // 获取活动列表
  const fetchActivities = useCallback(
    async (showLoading = true, category?: string, filter?: string, isLoadMore = false) => {
      try {
        if (showLoading) {
        ...
        if (res?.data) {
          const pageData: unknown = res.data;
          if (
            pageData &&
            typeof pageData === 'object' &&
            'items' in pageData &&
            Array.isArray((pageData as { items: unknown }).items)
          ) {
            if (currentFilter === '我报名的') {
              list = (pageData as { items: unknown[] }).items.map((item: unknown) => {
                if (item && typeof item === 'object' && 'activity' in item) {
                  return (item as { activity: ActivityRead }).activity;
                }
                return item as ActivityRead;
              });
            } else {
              list = (pageData as { items: ActivityRead[] }).items;
            }
          } else if (Array.isArray(pageData)) {
            list = pageData as ActivityRead[];
          }
        }
        ...
        if (currentFilterValue === '我报名的' || currentFilterValue === '我发布的') {
      ...
      setTimeout(() => {
        ...
        if (selectedCategory !== '全部') {
          finalFiltered = filtered.filter(
            (activityItem) => activityItem.category === selectedCategory,
          );
        }
  ...
  const handleJoinActivity = async (act: ActivityRead, event: unknown) => {
  ...
  const handleCancelRegistration = async (act: ActivityRead, event: unknown) => {
    (event as { stopPropagation: () => void }).stopPropagation();
  ...
  const toggleExpanded = (activityId: string, event: ClickEvent) => {
  ...
  return (
    <View className={styles.activitySquarePage}>
      ...
      <View className={styles.fixedHeader}>
            ...
            {filterOptions.map((filter) => (
              ...
              >
                ...
                {selectedFilter === filter && (
                  <Text
                    className={`${styles.filterArrow} ${showCategoryBar ? styles.filterArrowUp : ''}`}
                  >
                    ▼
                  </Text>
                )}
        ...
        {/* 活动分类 - 条件显示 */}
        {showCategoryBar && (
          <ScrollView scrollX className={styles.categoryScrollContainer} showScrollbar={false}>
        ...
        >
          {activitiesLoading ? (
            ...
            filteredActivities.map((act) => {
              ...
              const isRegistered =
                act.registration_status !== RegistrationStatus.cancelled && act.is_registered;
              const isFull =
                    ...
                    <View className={styles.titleSection}>
                      <Text className={styles.activityTitle}>
                        ...
                        {act.publisher_organization && (
                          <Text className={styles.organizerTagInline}>
                            {' '}
                            {act.publisher_organization}
                          </Text>
                        )}
