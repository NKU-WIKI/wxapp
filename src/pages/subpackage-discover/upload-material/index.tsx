...
import { BaseResponse, InputEvent } from '@/types/api/common';
...
export default function UploadMaterial() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<LearningMaterialCategory>(
  ...
  const [selectedFile, setSelectedFile] = useState<
    Taro.chooseMessageFile.SuccessCallbackResult['tempFiles'][0] | null
  >(null); // 本地选择的文件
  ...
  // 网盘链接输入
  const handleNetdiskInput = (e: InputEvent) => {
  ...
  // 学院选择
  const handleCollegeChange = (e: PickerEvent) => {
    const index = Array.isArray(e.detail.value)
      ? (e.detail.value as number[])[0]
  ...
  // 学科选择
  const handleSubjectChange = (e: PickerEvent) => {
    const index = Array.isArray(e.detail.value)
      ? (e.detail.value as number[])[0]
      : (e.detail.value as number);
  ...
  // 分类选择
  const handleCategoryChange = (e: PickerEvent) => {
    const index = Array.isArray(e.detail.value)
      ? (e.detail.value as number[])[0]
      : (e.detail.value as number);
  ...
  // 署名方式选择
  const handleSignatureChange = (e: InputEvent) => {
    ...
    Taro.chooseImage({
      ...
      success: async (res) => {
        ...
        try {
          ...
          Taro.showToast({
            title: (error as Error)?.message || '收款码上传失败',
            icon: 'none',
          });
        }
      },
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      let fileUploadResult: BaseResponse<FileUploadRead> | null = null;

      // 如果有选择文件，先上传文件
      if (selectedFile) {
        // 根据学科分类决定文件分类
        const category =
          selectedSubject.includes('计算机') || selectedSubject.includes('软件')
            ? 'code'
            : 'document';
        ...
        fileUploadResult = (await fileUploadApi.uploadFileSimple(
          ...
          category,
          false, // 默认不公开
        )) as { code: number; data: FileUploadRead };

        if (fileUploadResult && fileUploadResult.data) {
          setUploadedFile(fileUploadResult.data);
      ...
      const is413Error =
        ...
        (typeof errorObj?.errMsg === 'string' && errorObj.errMsg.includes('413')) ||
        (typeof errorObj?.message === 'string' && errorObj.message.includes('413')) ||
        (typeof errorObj?.message === 'string' && errorObj.message.includes('文件大小超过限制')) ||
        (typeof errorObj?.data === 'object' &&
          errorObj.data &&
          typeof (errorObj.data as Record<string, unknown>).message === 'string' &&
          String((errorObj.data as Record<string, unknown>).message).includes('413')) ||
        (typeof errorObj?.data === 'object' &&
          errorObj.data &&
          (errorObj.data as Record<string, unknown>).status === 413) ||
        errorObj?.status === 413;
  ...
  return (
    <View className={styles.uploadMaterialPage}>
          ...
          {uploadedFile ? (
            <View className={styles.uploadedFileInfo} onClick={handleSelectFile}>
              ...
              <Text className={styles.uploadedFileSize}>
                {getFileTypeDisplayName(uploadedFile.content_type || '')} -{' '}
            ...
            <View className={styles.selectedFileInfo} onClick={handleSelectFile}>
              ...
              <Text className={styles.selectedFileName}>{formatFileName(selectedFile.name)}</Text>
            ...
            <View className={styles.uploadEmptyState}>
              ...
              <Text className={styles.formatTip}>
                支持 PDF、Word、PPT 等格式，单个文件不超过 10MB
              </Text>
              <Button
      ...
      <View className={styles.formGroup}>
        ...
        <Picker mode='selector' range={colleges} rangeKey='name' onChange={handleCollegeChange}>
          <View className={styles.collegeSelect}>
            <Text className={styles.selectText}>
              {selectedCollege
                ? colleges.find((c) => c.value === selectedCollege)?.name
                : '请选择学院'}
            </Text>
      ...
      <View className={styles.formGroup}>
        ...
        >
          <View className={styles.subjectSelect}>
            <Text className={styles.selectText}>
              {selectedSubject
                ? getSubjectsForCollege(selectedCollege).find((s) => s.value === selectedSubject)
                    ?.name
                : '请选择学科'}
            </Text>
            <Text className={styles.selectArrow}>▼</Text>
      ...
      <View className={styles.formGroup}>
        ...
        >
          <View className={styles.categorySelect}>
            <Text className={styles.selectText}>{CATEGORY_CONFIG[selectedCategory].title}</Text>
            <Text className={styles.selectArrow}>▼</Text>
          </View>
        </Picker>
      </View>
