...
import {
  getResourceRatingsList,
  getResourceStatistics,
  createRating,
  updateRating,
} from '@/services/api/rating';
import { RatingStatistics, UserRating, ResourceRatingsResponse } from '@/types/api/rating';
...
const RatingDetailPage = () => {
  ...
  const [userExistingRating, setUserExistingRating] = useState<UserRating | null>(null);
  ...
  const loadData = useCallback(
    async (resourceType: string, resourceId: string, resourceName?: string) => {
      try {
        setLoading(true);
        setError(null);
        ...
        // 处理评分列表数据
        // API 返回 BaseResponse<RatingApiResponse<ResourceRatingsResponse>>
        let ratingsData: UserRating[] | null = null;
        if (ratingsResponse.data?.data?.items) {
          // 标准响应格式: { data: { data: { items: [...] } } }
          ratingsData = ratingsResponse.data.data.items;
        } else if ((ratingsResponse.data as unknown as ResourceRatingsResponse)?.items) {
          // 直接响应格式: { data: { items: [...] } }
          ratingsData = (ratingsResponse.data as unknown as ResourceRatingsResponse).items;
        }
        ...
        if (ratingsData && Array.isArray(ratingsData)) {
          ...
          if (isLoggedIn && userState.user?.id) {
            const existingRating = ratingsData.find(
              (rating) => rating.rater_id === userState.user?.id,
            );
            if (existingRating) {
          ...
          if (firstRating) {
            const resourceInfo = {
              ...
              title:
                firstRating.resource_title ||
                firstRating.resource_name ||
                resourceName ||
                '未知资源',
              ...
              average_score:
                statisticsResponse.data?.data?.averageRating ||
                (statisticsResponse.data as { averageRating?: number })?.averageRating ||
                0,
              rating_count:
                statisticsResponse.data?.data?.totalRatings ||
                (statisticsResponse.data as { totalRatings?: number })?.totalRatings ||
                0,
            };
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      // 处理409冲突错误（已评分）
      if (
        (err as { statusCode?: number; code?: number }).statusCode === 409 ||
        (err as { statusCode?: number; code?: number }).code === 409
      ) {
        Taro.showToast({
    ...
    <View className={styles.ratingDetailPage}>
      ...
      <ScrollView scrollY className={styles.contentScroll}>
          ...
          <View className={styles.ratingDistribution}>
            ...
            <View className={styles.distributionList}>
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                const percentage =
                  statistics.totalRatings > 0 ? (count / statistics.totalRatings) * 100 : 0;
                return (
        ...
        <View className={styles.userRatingForm}>
          <Text className={styles.sectionTitle}>{userExistingRating ? '更新评价' : '写评价'}</Text>
          {userExistingRating && (
          ...
          <View className={styles.ratingInput}>
            ...
            <View className={styles.ratingStars}>{renderRatingStars()}</View>
          </View>
          ...
          >
            <Text className={styles.submitText}>
              {submitting ? '提交中...' : userExistingRating ? '更新评价' : '发布评价'}
            </Text>
          </View>
        </View>
                ...
                <View key={rating.id} className={styles.reviewItem}>
                  <View className={styles.reviewHeader}>
                    ...
                    <View className={styles.userInfo}>
                      ...
                      <View className={styles.reviewStars}>{renderStars(rating.score)}</View>
                    </View>
                    <Text className={styles.reviewDate}>{formatDate(rating.created_at)}</Text>
                  </View>
                  ...
                  {rating.tags && rating.tags.length > 0 && (
                    <View className={styles.reviewTags}>
                      {rating.tags.map((tag: string) => (
                        <Text key={tag} className={styles.reviewTag}>
                          {tag}
                        </Text>
                      ))}
                    </View>
